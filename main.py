...
async def support_send_message_1_func(message: types.Message, state: FSMContext):
    ...
    if message.text == cfg.BACK_BUTTON(lang):
        ...
        if message.text:
            ...
            if message.caption:
                await bot.send_photo(cfg.SUPPORT_GROUP, caption=f"{cfg.USER_SEND_TASK_TEXT(user=fnc.nick_with_link('Օգտատերը', user_id), user_id=user_id)}\n\n{message.text}", photo=message.photo, parse_mode=types.ParseMode.MARKDOWN)
                await message.answer(cfg.TAKE_TEXT_SUPPORT(lang), reply_markup=markup)
                await state.finish()
            else:
                await bot.send_photo(cfg.SUPPORT_GROUP, photo=message.photo, parse_mode=types.ParseMode.MARKDOWN)
                await message.answer(cfg.TAKE_TEXT_SUPPORT(lang), reply_markup=markup)
                await state.finish()
        else:
